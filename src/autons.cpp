...
void long4middle3right() {//lowkey goated auton
  ...
  bottomIntake.move(127);
  chassis.pid_wait();
  chassis.pid_odom_set({{-17_in, 34_in}, fwd, 90});
  chassis.pid_wait();
  pros::delay(800);
  topIntake.move(0);
  middleIntake.move(0);
  bottomIntake.move(0);
  wings.set(false);
  chassis.pid_odom_set({{23_in, 35.0_in}, rev, 100});
  chassis.pid_wait_quick();
  topIntake.move(-127);
  middleIntake.move(127);
  bottomIntake.move(127);
  ...
  middleIntake.move(127);
  bottomIntake.move(127);
  chassis.pid_odom_set({{31.5_in, 5.5_in}, fwd, 50});
  pros::delay(1100);
  /*scraper.set(true); //it works without scraper since i slowed it from 70 to 50
  pros::delay(250);
  scraper.set(false);*/
  ...
  chassis.pid_wait_quick();
  chassis.pid_odom_set({{39.51_in, -2_in}, rev, 90});
  chassis.pid_wait_quick();
  chassis.pid_wait();
  ...
  topIntake.move(127);
  pros::delay(2000);
  chassis.pid_odom_set({{16_in, 21_in}, fwd, 90});
  chassis.pid_wait_quick();
  chassis.pid_turn_set(-90,90);
  chassis.pid_drive_set(-24_in,127);
  chassis.pid_wait();
...
void sawp(){//works pretty well try not to change anything
  ...
  chassis.pid_wait();
  chassis.pid_odom_set({{-23.5_in, -41_in}, fwd, 127});//scrap more balls
