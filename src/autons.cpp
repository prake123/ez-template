...
void long4middle3right() {//lowkey goated auton
  ...
  topIntake.move(-127);
  pros::delay(2000);
  chassis.pid_odom_set({{-16_in, 21_in}, rev, 120});
  chassis.pid_wait_quick();
...
void sawp(){//works pretty well try not to change anything
  ...
  chassis.pid_wait();
  chassis.pid_odom_set({{-23.5_in, -38.5_in}, fwd, 127});//scrap more balls
  ...
  middleIntake.move(0);
  bottomIntake.move(0);
  chassis.pid_odom_set({{{0_in, -64.5_in}, fwd, 127}});//go towards match loader
  topIntake.move(-127);
  middleIntake.move(-127);
  ...
  middleIntake.move(127); 
  bottomIntake.move(127);
  chassis.pid_odom_set({{17_in, -65.5_in}, fwd, 80});//
  // scrape match loader
  scraper.set(true);
